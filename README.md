...
- Paste command dibawah untuk memasang script ``jam.sh``
    ...
    Menggunakan **`wget`**
    ```
    wget --no-check-certificate "https://raw.githubusercontent.com/xodpool/Fixjamwithbug/refs/heads/main/jam.sh" -O /usr/bin/jam.sh && chmod +x /usr/bin/jam.sh
    ```
    Menggunakan **`curl`**
    ```
    curl -sL raw.githubusercontent.com/xodpool/Fixjamwithbug/refs/heads/main/jam.sh > /usr/bin/jam.sh && chmod +x /usr/bin/jam.sh
    ```
